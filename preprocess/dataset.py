...
class SiamessDataset(Dataset):
    def __init__(self, args, nl_corpus_path, nl_vocab, nl_seq_len, path_vocab,
        ...
        self.code['paths'] = self.df2vocab(paths, max_len=self.path_seq_len,id_vocab=path_vocab)

        # tokens
        tokens = pd.read_csv(file_path('tokens', True), sep=',')
        #print(tokens)
        self.code['tokens'] = self.df2vocab(tokens, id_vocab=nl_vocab)
        self.token_map = self.df2vocab(tokens[['id', 'token_cut']],max_len=self.nl_seq_len,id_vocab=nl_vocab)
        ...
        print('长度',len(self.code['path_contexts']))
        print('===')
