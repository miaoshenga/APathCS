...
class MultiHeadAttention(nn.Module):
    ...
    def jointAttention(self,Q,K,V):
        ...
        context = torch.matmul(attn.transpose(-1, -2), v_s)
...
class Embedder(nn.Module):
    ...
    def forward(self, nl, path, start_token, end_token):
        ...
        code_outputs = torch.cat((start_token_emb, end_token_emb, path_emb), dim=1)

        code_repr = self.self_att(code_outputs, code_outputs, code_outputs)


        feat_desc = self.self_att.jointAttention(nl_outputs,code_repr,nl_outputs)
        feat_code = self.self_att.jointAttention(code_repr,nl_outputs,code_repr)

        code_repr = torch.mean(self.linear(feat_code), dim=1)
        desc_pos_repr = torch.mean(self.linear(feat_desc), dim=1)
